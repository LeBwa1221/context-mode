...
function getStore(): ContentStore {
  if (!_store) {
    ...
    // #985: keep the shared content-store WAL bounded — close() no longer
    // checkpoints either (#1056), so PASSIVE here is the only WAL-bounding
    // path. PASSIVE never blocks and adds no locking (ADR-0001-safe).
    // Default 60s; CONTEXT_MODE_WAL_CHECKPOINT_MS tunes it, 0 disables.
    {
...
// Exported only for tests/session/fetch-page-store-path.test.ts, which
// guards against getPageStore() ever regressing off getStorePath() and back
// onto a profile/config-scoped path (see HANDOFF.md items 6-7).
 ...
 * (PRD finding E).
 */
export function indexFetched(f: { url: string; source?: string; markdown: string; header: string; route?: FetchRoute; rung?: string }): IndexedFetchResult {
  const store = getStore();
