...
function getStore(): ContentStore {
  if (!_store) {
    ...
    // #985: keep the shared content-store WAL bounded even on a hard exit
    // that never reaches close()'s TRUNCATE checkpoint. PASSIVE never
    // blocks and adds no locking (ADR-0001-safe). Default 60s;
    // CONTEXT_MODE_WAL_CHECKPOINT_MS tunes it, 0 disables.
    {
