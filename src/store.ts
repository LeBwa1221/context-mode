...
import { loadDatabase, applyWALPragmas, closeDB, cleanOrphanedWALFiles, withRetry, deleteDBFiles, isSQLiteCorruptionError, startWalCheckpointTimer, reopenAfterHeal } from "./db-base.js";
...
// while legacy oversized rows (pre-dating MAX_CHUNK_BYTES) are hydrated as a
// bounded match window instead (adopted from upstream #963, #947 — a 53MB
// legacy row turned a millisecond MATCH into a multi-minute synchronous
// highlight()).
...
export class ContentStore {
  ...
  /**
   * Heal a mid-session corrupt content DB (adopted from upstream #871, #867).
   * Closes the held handle, attempts lossless recovery, reopens, and
   * re-initializes schema + prepared statements so the next write retry
   * operates on a healthy connection.
   */
   ...
   * + retry instead of surfacing the same error on every subsequent write.
   */
  #withRetry<T>(fn: () => T): T {
  ...
  /**
   * Hydrate a ranked metadata-only row with actual content. Normal
   * (<=MAX_CHUNK_BYTES) rows get the historical full-content + FTS5
   * highlight() result; legacy oversized rows (pre-dating the cap) are
   * hydrated as a bounded match window instead — avoids materializing a
   * multi-MB row through highlight() (adopted from upstream #963, #947).
   */
  #hydrateSearchRow(
    row: SearchRow,
    query: string,
