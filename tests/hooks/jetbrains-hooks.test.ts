...
describe("JetBrains Copilot hooks — MCP cwd != hook projectDir worktree-suffix (#435)", () => {
  ...
  beforeAll(async () => {
    ...
    // ~/.config/JetBrains/context-mode, unaffected by session storage.
    // Session storage itself is the shared global root now
    // (maint/integration).
    const debugLogDir = join(homedir(), ".config", "JetBrains", "context-mode");
    const sessionsDir = join(resolveContextModeDataRoot(), "context-mode", "sessions");
